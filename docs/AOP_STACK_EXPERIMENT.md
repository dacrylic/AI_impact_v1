...
4. Fit a regularized ridge-classifier meta-model on those
   compound scores.
...
clean CPU method screen. A later candidate that materially improves the locked
baseline can justify more expensive cross-fitted stacking.
...
The remaining worthwhile variant would be to add A/O/P specialist margins as
supplementary cross-fitted features to the title-plus-structured-task baseline,
not to replace that baseline with the components alone.
